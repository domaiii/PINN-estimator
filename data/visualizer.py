...
class Visualizer:
    ...
    def add_scalar_field(
        ...
        dofmap = scalar_func.function_space.dofmap

        # Cellwise constants (for example DG0) must be plotted as face values.
        if len(dofmap.cell_dofs(0)) == 1:
            cell_values = np.asarray(
                [scalar_func.x.array[dofmap.cell_dofs(i)[0]] for i in range(len(self.cells))]
            ).real
        ...
        V_plot = fem.functionspace(
            self.mesh, element("Lagrange", self.mesh.basix_cell(), 1)
        )
